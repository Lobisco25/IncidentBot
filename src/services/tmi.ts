...
const joinChannels = async () => {
    ...
    channelsTable.map((c) => c.twitch_name).forEach(async (channel) => channels.push(channel));
    client.joinAll(channels).catch((err) => {
...
client.on("ready", async () => {
    client.privmsg(config.mainChannel, `${dev} ${config.restartEmote} ${reconnected} (${await ping()})`);
