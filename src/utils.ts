...
utils.formatMS = (asd: number, options: prettyMilliseconds.Options) => {
    return prettyMilliseconds(asd, options);
};
...
utils.getEmote = async (channel: string, emotes: string[], emoji: string) => {
    ...
    emotes.forEach((element) => {
        if (res.emote_set.emotes.some((e: { name: any }) => e.name === element)) {
            AvailableEmotes.push(element);
        }
    });
