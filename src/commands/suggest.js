...
exports.run = async (client, msg, args, cmd) => {
    ...
    const check = async () => {
        ...
        return `suggestion "${suggestion[0].id}": "${suggestion[0].desc}" | status: ${suggestion[0].status} | category: ${suggestion[0].category ?? "none"} ${suggestion[0].comment ? `| comment: "${suggestion[0].comment}"` : ' '}`
    }
    ...
    const update = async () => {
        ...
        await db("suggestions").where({ id: args[1] }).update({ desc: args.slice(2).join(" ") })
    ...
    switch (arg) {
        default: return await defaultCase()
        case "check": return await check()
        case "delete": return await deleteCase()
